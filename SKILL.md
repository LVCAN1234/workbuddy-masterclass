...
description: |
  WorkBuddy 从零到精通：19 节核心课 + 4 个补充板块（领域知识库 / 实战工作台 / 集成式生活工作台 / 建站上云实战）系统掌握 AI Agent。面向零基础用户（AI小白），通过"理论讲解 + 图解 + 互动验收"的方式教学。涵盖 Agent 原理、LLM工作机制、Function Calling、记忆系统、Skill开发、MCP连接器、自动化任务、竞品对比、MCP实操、工作日报实战、创作者路径等全部核心能力。
  触发场景：用户表示想学 Agent、想了解 WorkBuddy、问"什么是 Agent"、"教我用 WorkBuddy"、"Agent 入门"、"WorkBuddy 能做什么"、"AI Agent 课程"等。
  课程共 19 节核心课 + 4 个补充板块：第0课WorkBuddy界面导览 + 基础课（1-8节）+ 进阶课（9-18节）+ 补充板块（19 领域知识库、20 实战工作台、21 集成式生活工作台、22 建站上云实战），包含实战项目（搭建 MiniTask Agent）、MCP 实操、自动日报实战、GEO 工作台演示、生活工作台案例和创作者路线。
author: 灿哥（老板）
category: education
version: 1.0.0
tags:
...
### 补充板块（19-22 节）：知识库 + 工作台 + 生活工作台 + 建站实战
...
| 21 | 集成式生活工作台 | 碎片化痛点→一站式工作台、单文件 HTML 四支柱、9 大生活模块、怎么造怎么用（以「灿哥的生活工作台」为案例） |
| 22 | 建站上云实战 | 四步全链路（出网站→租服务器→部署上线→日常维护）、以谋仕AI官网为真实案例、串起 Agent/四件套/Function Calling/知识库回写 |
...
2. **设定预期**：告知课程 19 节核心课 + 4 个补充板块（含第 0 课界面导览）、分基础和进阶两个阶段，外加知识库 / 工作台 / 生活工作台 / 建站实战四个补充板块
...
3. **知识库板块 / 工作台板块 / 生活工作台板块**：已作为第 19、20、21 课正式做进课程（详见 `references/curriculum.md`），综合三部分内容落地 L3 领域知识层 + L4 教学专家。
...
### 演示素材：灿哥的生活工作台（第 21 课真实案例）

第 21 课用一个真实上线的「集成式生活工作台」单文件 HTML，作为第 20 课「工作台」概念的生活化落地：

```
assets/life-workbench.html
```

这是一个纯前端、离线可用的个人生活统筹台：
- 9 大模块：今日热点（时事 + AI）、记账理财（含导出）、习惯健康打卡 + 30 天热力图、减脂健身、日程统筹、待买清单、爆款视频收藏、今日待办（工作 / 生活）、待取快递
- 数据存浏览器本地（localStorage），即时保存；JSON 一键导入导出备份；满 20 条提醒备份；损坏可恢复
- 单文件交付：CSS / JS / 图标全内联，双击即开，不依赖服务器或账号

教学时应：直接双击打开给学员看「一个页面看全天」，再对照第 21 课讲「碎片化痛点→一站式工作台」与五步搭建法。

### 演示素材：谋仕AI 官网（第 22 课真实案例）

第 22 课用学员真实上线的企业官网（geomoushi.com）作为「建站 + 上云 + 绑域名」全链路的贯穿案例，无需额外素材文件。
...
- `references/curriculum.md` — 19 节核心课 + 4 个补充板块的详细教案（知识点清单、图解要点、验收题及答案，含第 19、20、21、22 课）
...
- `assets/life-workbench.html` — 第 21 课 集成式生活工作台演示素材（「灿哥的生活工作台」单文件 HTML）
